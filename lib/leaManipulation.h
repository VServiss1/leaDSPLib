...
#define NewLEA_VAR(var) LEA_EPRG(RETAIN(var)) LEA_EPRG(DATA_SECTION(var, ".leaRAM")) // This sets a new lea variable this is meant for GCC MSP430 Compiler
#define Q15(x) short ((x)<1.0? ((x)>-1.0? (x)*0x8000:0x8000):0x7FFF) // This is soley to turn decimal numbers into actual usable varaible for LEA
...
#define leaBottomMem (*((const uint16_t *)(&LEAMB))) //checks lea top
...
typedef enum{
    ...
    sizeError = 1,
    shiftSizeError = 2,
    tableSizeError = 3,leaBusy = 4,
    leaInvalidAddress = 5,leaOutOfRange = 6,
...
bool codeInRam();
/*
This is code to check LEA done or lea free
these are seperate from the rest of the status checks as this is using leaflags while the other is using lea status
*/
bool leaFree(void);
bool leaDone(void);
...
LEA_STATUS addMatrixLong(signed long *x, signed long *y, signed long *outputZ,
                         short offsetX, short offsetY, short offsetZ, unsigned short size);
LEA_STATUS subMatrix(signed short *x, signed short *y, signed long *outputZ,
                     short offsetX, short offsetY, short offsetZ, unsigned short size);
...
LEA_STATUS macBase(signed short *x, signed short *y, signed long *outputZ, unsigned short size); // this creates an array of memory addresses and reserved bits to help
LEA_STATUS macThree(signed short *x, signed short *y, signed long *outputZ, unsigned short size);
LEA_STATUS macSF(signed short *x, signed short *y, double SF, signed long *outputZ, unsigned short size); // MAC 3 with a scale factor
...
LEA_STATUS unsignedMinVectorLong(unsigned long *x, signed long outputZ, signed short offset, short size);
// seperator for group
LEA_STATUS maxVectorDual(signed short *x, signed long outputZ, signed short offset, short size);           // HATE HATE HATE
LEA_STATUS unsignedMaxVectorDual(unsigned short *x, signed long outputZ, signed short offset, short size); // HATE HATE HATE
LEA_STATUS minVectorDual(signed short *x, signed long outputZ, signed short offset, short size);           // HATE HATE HATE
LEA_STATUS unsignedMinVectorDual(unsigned short *x, signed long outputZ, signed short offset, short size); // HATE HATE HATE
